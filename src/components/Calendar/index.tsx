...
export function Calendar({ selectedDate, onDateSelected }: CalendarProps) {
	...
	const calendarWeeks = useMemo(() => {
		...
		const previousMonthFillArray = Array.from({
			length: firstWeekDay
		}).map((_, i) => currentDate.subtract(i + 1, "day")).reverse();
		...
		const calendarWeeks = calendarDays.reduce<CalendarWeeks>(
			(weeks, _, i, original) => {
				const isNewWeek = (i % 7) === 0; // % é o resto da divisão, então toda a vez que chegou a 7, precisa ir para a proxima semana, 
